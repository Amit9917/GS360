...
subtitle: "RIS Software Maintenance & Support Services"
author: "Jina Code Systems"
date: "March 2026"
geometry: margin=1in
fontsize: 11pt
...
**Prepared by**\
Jina Code Systems\
855, Spaze iTech Park, Sector 49, Gurugram\
March 2026
...
- Will be addressed on a best-effort basis and are not subject to the resolution timelines defined in Section 6
...
- Client-side modifications to the system (see Section 8)
...
Monday to Friday, 09:30 AM to 5:30 PM (IST)

**5.2 Emergency Support**

24/7 support available only for Severity 1 (Critical) issues. Emergency support outside standard business hours may be charged at ₹3,000/hour if the issue is subsequently determined to be caused by Client infrastructure, unauthorized changes, or factors outside Provider's application scope.
...
| Severity 4 — Low | Cosmetic or UI issues, non-critical improvements | Within 2 working days | Next planned release cycle |

Important notes on the above timelines:

- All response and resolution times are target and best-effort timelines, not guaranteed commitments. The Provider will make commercially reasonable efforts to meet these targets.
...
The clock resumes upon receipt of the required information or access.
...
- If the Client exceeds the monthly support cap, the Provider reserves the right to:
  - Charge additional hours at ₹2,000/hour
...
- Would require effort disproportionate to the SLA fee

In such cases, the Provider will communicate the rationale and may offer to address the request as a Change Request (Section 12).
...
Only issues logged through official support channels (email or ticketing system) will be tracked and counted under this SLA. Issues communicated verbally, via personal messaging, or informal channels will not be considered valid SLA tickets unless subsequently logged through official channels.

Each ticket must include: issue description, severity assessment, steps to reproduce, and relevant screenshots or logs.
...
- Provide detailed issue descriptions with screenshots and logs where applicable
...
- External system or API failures beyond Provider's control
...
The Provider will follow industry-standard security practices including encrypted remote access (SSH/VPN), secure credential handling (no plaintext storage of credentials), principle of least privilege for all access, and multi-factor authentication where supported by Client infrastructure.

The Provider is not responsible for data backups or disaster recovery, data loss caused by infrastructure failures, Client actions, or external factors, or security breaches resulting from Client's failure to maintain secure infrastructure.


## 12. Change Requests & New Features

Any requirement outside the defined SLA scope — including but not limited to new feature development, changes in existing workflows, additional reports or dashboards, integration with new systems, database schema changes or migrations, and any enhancement exceeding the limits defined in Section 2.5 — will be treated as a Change Request (CR).
...
- Intellectual property created as part of Change Requests shall be owned by the Provider, unless explicitly agreed otherwise in writing for a specific CR
...
- Provider shall not be liable for indirect, incidental, consequential, special, or punitive damages of any kind; loss of revenue, profits, data, or business opportunities; business interruption or operational losses; data loss due to infrastructure failures, Client actions, or external factors; delays caused by Client's failure to provide timely access, information, or approvals; issues arising from unauthorized changes made by Client or third parties; or damages arising from the Client's reliance on the Provider's recommendations
...
- Neither party shall be liable for any delay or failure to perform obligations under this Agreement due to force majeure events including but not limited to natural disasters, pandemics, epidemics, war, terrorism, riots, government orders, widespread internet or power outages, or any event beyond the reasonable control of the affected party
...
All formal notices under this Agreement (including payment reminders, termination notices, and dispute communications) shall be delivered via email to the designated contact addresses of each party, and via registered post or courier to the registered address of the party.

A notice shall be deemed delivered on the date of email delivery (with read receipt or delivery confirmation), or 3 business days after dispatch via registered post or courier, whichever is earlier.

Designated contacts:

- Provider: Amit Joshi, COO — _________________________ (email)
- Client: _________________________ — _________________________ (email)
...
Acknowledged by Provider: _________________________ Date: ___________

Acknowledged by Client: _________________________ Date: ___________
